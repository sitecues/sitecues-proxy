...
function getReleaseLib (release) {
    ...
    var releaseArray = release.split('.'),
        ...
        patch        = releaseArray[2];

    return '/v/release-' + major + '.' + minor + '/' + major + '.' + minor + '.' + patch + '-RELEASE/js/sitecues.js';
}
...
function getLoadScript(options) {
    ...
    if (verbose) {
        console.log('Injecting sitecues script: ' + scriptUrl);
    }
    ...
    '            sitecues.config.site_id = \''+ siteId + '\';\n'                      +
    '            (function () {\n'                                                    +
