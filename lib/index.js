/*
 * This is the main file of our application that contains all the functions we need.
 */
...
function onResponse(request, response) {
    ...
    if (util.isEligible(request)) {
        ...
        if (reverseMode) {
            ...
            // Detect HTTP redirect responses, which would send a user away from the proxy.
            if (['301', '302', '303', '307', '308'].indexOf(response.statusCode.toString()) >= 0) {
                log.ok('Encountered an HTTP', response.statusCode, 'redirect code, pointing to:', response.headers.location);
                log.ok('Re-writing the redirect to:', makeProxyUrl(url.resolve(target, response.headers.location)));
                // Re-write HTTP redirects to use the proxy. These can be relative, so we must resolve them from the original target.
                response.headers.location = makeProxyUrl(url.resolve(target, response.headers.location));
            }
            ...
            //     var result = value;
            //     if (value.length > 0) {
            //         log.ok('Came across a stylesheet...');
            //         log.ok('Original href :', value);
            //         // Bypass the proxy entirely for downloading stylesheets.
            //         result = url.resolve(target, value);
            //         log.ok('Final href    :', result);
            //     }
            // });
