...
var pkg          = require('../package.json'),  // project metadata
    ...
    defaultPorts = require('./default-ports'),  // port constants
    alignJson    = require('json-align'),       // JSON prettifier
    url          = require('url'),              // URL formatting utility
    log          = require('./util').log,
    MIN_PORT     = 1024,   // OS X and *nix require sudo for <1024
    MAX_PORT     = 65535,  // most systems have this as an upper limit
...
function onHtmlResponse(request, response) {
    ...
    if (this.state.removeLoadScript) {
...
function onListening() {
    ...
    // This function is run when the proxy is up and ready
...
function startProxy(error, foundPort) {
    ...
    if (error) {
        ...
        proxy.log('error warn debug', function (event) {
            ...
            var errTypeIndex = event.message.search(/\w*?(?=:)/);
            // Hoxy throws in some useful data (like the phase), before the error
            // type, so let's keep that.
            var message = event.message.substring(0, errTypeIndex);
            if (message) {
        ...
        proxy.listen(
            ...
            onListening  // callback to run when the proxy is ready
        );
    }
}
...
function toProxiedUrl(target) {
    ...
    if (state.reverseMode) {
        result = state.origin      +  // 'scheme://hostname:port' for the proxy
                 state.contextPath +
                 state.route.page  +
                 target;
    }
...
function Server(options) {
    ...
    // Here we tell the proxy which webpage is the default target when it
    // enters reverse mode. We override this for each request and you
    // should never actually see it, but it is necessary configuration
    // to actually enter reverse mode in the underlying API.
    if (state.reverseMode) {
