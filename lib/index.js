    ...
    if (util.isEligible(request)) {
        ...
        if (reverseMode) {
            ...
            response.$('a[href]').attr('href', function (index, value) {
                ...
                console.log('Came across an anchor...');
                console.log('Original href :', value);
    ...
    if (!alreadyUp) {
        if (options) {
            ...
            if (typeof port === 'string' && options.port.trim().toLowerCase().indexOf('auto') === 0) {
                ...
                portscanner.findAPortNotInUse(port, MAX_PORT, hostname, startProxy);
            }
            else {
                ...
                portscanner.checkPortStatus(port, hostname, function (error, status) {
                    ...
                    // A status of open means the port is busy, closed means it can be used.
                    else if (status === 'closed') {
                        startProxy.call(proxy, undefined, port)
                    }
                    else {
                        log.error(
                            'Port ' + port + ' is not available. Use PORT=\"auto\" ' +
                            'to have the proxy use one that is.'
                        );
                        process.exit(1);
                    }
                });
