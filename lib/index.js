...
var pkg             = require('../package.json'),  // metadata about the proxy application
    ...
    fs              = require('fs'),               // utility to create and control the proxy server
...
function onHtmlResponse(request, response) {
    ...
    if (reverseMode) {
        ...
        // Details: https://equinox.atlassian.net/browse/SC-2556
        //$('head').prepend(
...
function startProxy(error, foundPort) {
    ...
    if (error) {
        ...
        if (reverseMode) {
            ...
            proxy = new hoxy.Proxy({
              certAuthority: {
                key: fs.readFileSync('keys/sitecues-proxy.key.pem'),
                cert: fs.readFileSync('keys/sitecues-proxy.crt.pem')
              }
            });
        }
...
function start(options) {
    ...
    loadScript = util.getLoadScript(options);        // the sitecues loader, which the proxy will insert
