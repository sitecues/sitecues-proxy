...
function listen() {
    ...
    badCondition = state.isStarting ? 'starting' : (state.isStopped ? 'started' : false);
...
function close() {
    ...
    badCondition = state.isStopping ? 'stopping' : (state.isStopped ? 'stopped' : false);
...
function Server(options) {
    ...
    if (this.state.reverseMode) {
        this._proxy = new hoxy.proxy(
            {
                reverse : this.state.target
            }
        );
    }
    else {
