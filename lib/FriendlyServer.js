...
const isValidPortNumber = (port) => {
    return port === minPort ||
           port === maxPort ||
           (minPort < port && port < maxPort);
};

const assertValidPortNumber = (portInt, originalInput) => {
    if (!isValidPortNumber(portInt)) {
        throw new RangeError(
                `${originalInput} is not a valid port number of ${minPort} - ${maxPort}.`
            );
    }
};
...
class FriendlyServer extends Server {
    ...
    constructor(options) {
        const perCallDefault = {
            ...
            proxyLinks     : process.env.PROXY_LINKS,
