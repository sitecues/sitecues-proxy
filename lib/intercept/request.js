...
function onRequest(request, response) {
    ...
    if (state.reverseMode) {
        ...
        const targetCruft = new RegExp(
            // TODO: Don't make so many assumptions here. This regex must match
            //       a '/' at the beginning even if contextPath doesn't have one.
            //       We also need to match exactly one '/' between contextPath
            //       and route.page, even if contextPath ends with one and
            //       route.page starts with one. Basically, we need something
            //       like path.posix.join() but then to insert the optional
            //       non-capturing group syntax at the correct string indexes.
