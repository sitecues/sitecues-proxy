...
function onHtmlResponse(request, response) {
    ...
    if (state.reverseMode) {
        ...
        $('base[href]').first().attr('href', function (index, value) {
            ...
            const
                result = state.proxyBase ?
                    that.toProxiedPath(absoluteUrl) :
                    absoluteUrl;

            return result;
        });
        console.log('proxy base:', state.proxyBase);
        console.log('proxy links:', state.proxyLinks);
        const
            ...
            baseUrl = state.proxyBase ? proxiedBaseUrl : targetBaseUrl;
