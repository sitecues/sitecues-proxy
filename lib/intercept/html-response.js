...
const
    ...
    defaultSiteId = 's-00000005', // Note:'s-98595d91' should be used for the reverse proxy to separate testing and reverse proxy metrics
    defaultJsHostName = 'localhost';

// Replace any templated variables that need replacing, e.g. {{{autoSiteId}}}
function getFinalLoader(requestedLoader, existingLoaders) {
    function getAutoSiteId() {
        if (!existingLoaders || existingLoaders.length === 0) {
            return defaultSiteId;
        }
        let detectedSiteId = existingLoaders.text().match(/\'(s-[0-9a-z]{8})\'/);
        return detectedSiteId ? detectedSiteId[1] : defaultSiteId;
    }

    function getHostName() {
        return process.env.JS_HOSTNAME || defaultJsHostName;
    }

    function getVersion() {
        var requestedVersion = process.env.VERSION,
            branchName;
        ...
        if (branchName === 'dev') {
            // dev branch uses [VERSION]-dev for version parameter
            return requestedVersion + '-DEV';
        }

        // Other branches use [VERSION]-BRANCH-[UPPERCASE_BRANCH_NAME]
        return requestedVersion + '-BRANCH-' + branchName.toUpperCase();
    }
    ...
    return requestedLoader
        .replace('\{\{\{SITE_ID\}\}\}', getAutoSiteId())
        .replace('\{\{\{JS_HOSTNAME\}\}\}', getHostName())
        .replace('\{\{\{BRANCH\}\}\}', getBranch())
        .replace('\{\{\{RAWVERSION\}\}\}', process.env.VERSION)
...
function getExistingLoaders($) {
    ...
    // Strategy 2: find a script with a src that contains our comment, or the src contains "/sitecues-loader.js"
    function hasSitecuesScriptComment(text) {
        return text && text.indexOf('// DO NOT MODIFY THIS SCRIPT WITHOUT ASSISTANCE FROM SITECUES') >= 0;
    }

    function hasSitecuesNamedScript(src) {
        return src && src.indexOf('/sitecues-loader.js') >= 0;
    }

    return $('script').filter(function(index, elem) {
        var $elem = $(elem);
        return hasSitecuesScriptComment($elem.text()) ||
            hasSitecuesNamedScript($elem.attr('src'));
    });
}
...
function onHtmlResponse(request, response, cycle) {
    ...
    const existingLoaders = getExistingLoaders($);
    if (existingLoaders.length) {
        console.log ('Found existing %d loaders', existingLoaders.length);    
    }
    
    let newLoader;

    if (state.loaderStrategy === loaderStrategy.REMOVE) {
        existingLoaders.remove();
    }
    else {

        newLoader = getFinalLoader(state.loader, existingLoaders);
        if (state.loaderStrategy === loaderStrategy.ADD) {
            $('head').first().prepend(newLoader);
        }
        else if (state.loaderStrategy === loaderStrategy.KEEP) {
            if (existingLoaders.length < 1) {
                // Insert before the <head> so that we load before any dynamically added
                $('head').first().prepend(newLoader); 
            }
        }
        else {
            if (existingLoaders.length > 0) {
                existingLoaders.first().replaceWith(newLoader);
            }
            else {
                $('head').first().prepend(newLoader);
            }
