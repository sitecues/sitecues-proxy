...
// TODO: In addition to adding the proxy origin and *.sitecues.com, we probably need the
//       target site's origin. This is because some absolute requests to the target may
//       slip by us in client-side code.
...
// https://developer.mozilla.org/en-US/docs/Web/Security/CSP/CSP_policy_directives#Source_lists

// TODO: Find out if different directives need to be edited for 'self' and *.sitecues.com

// TODO: Get the proxy origin value as computed by the actual request.
...
const getDirectiveAdder = (directive) => {
    const addDirective = () => {
...
const removeDirective = (directive, csp) => {
    csp.replace();
};
...
const addTo = (headers) => {
    ...
    if (csp.includes('script-src')) {
        headers[HEADER_NAME] = csp.replace(
            'script-src',
            `script-src ${PROXY_ORIGN} ${SC_ORIGIN}`
        );
    }
    ...
    if (csp.includes('connect-src')) {
        headers[HEADER_NAME] = csp.replace(
            'connect-src',
            `connect-src ${PROXY_ORIGN} ${SC_ORIGIN}`
        );
    }
};
